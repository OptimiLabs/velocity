...
import { ArrowLeft, Terminal, GitBranch, Network, Clock3, Timer, Sparkles, Info } from "lucide-react";
...
): "Priced" | "Mixed" | "Unpriced" {
  const normalized = (status || "").trim().toLowerCase();
  if (normalized === "unpriced") return "Unpriced";
...
function getProviderLines(
  ...
  if (provider === "codex") {
    return [
      "Reasoning tokens are tracked separately and included in billable output/cost.",
      cacheWriteUnavailable
        ? "Cache write tokens show N/A when Codex logs omit that metric."
        : "Cache read/write tokens come from token_count events when present.",
      "Per-message usage is reconstructed from token_count deltas.",
    ];
  }
  if (provider === "gemini") {
    return [
      "Input/output/cached token usage is read from model-turn usage fields.",
      "Cache write and explicit reasoning token fields are often not present.",
      "If usage cost is missing, cost is estimated from model pricing.",
    ];
  }
  return [
    "Input/output/cache usage is read from message usage when present.",
    "Thinking blocks are visible in transcript; separate reasoning token fields may be absent.",
    "If usage cost is missing, cost is estimated from model pricing.",
  ];
}
...
function hasUsageKey(
  usage: Record<string, unknown> | undefined,
  keys: string[],
): boolean {
  if (!usage) return false;
  return keys.some((key) => key in usage);
}

function getLoadedCacheTotals(messages: unknown[]): {
  read: number;
  write: number;
  writeFieldSeen: boolean;
} {
  let read = 0;
  let write = 0;
  let writeFieldSeen = false;
  ...
  for (const message of messages) {
    ...
    if (hasUsageKey(usage, writeKeys)) writeFieldSeen = true;
    write += readUsageTokenValue(usage, writeKeys);
  }

  return { read, write, writeFieldSeen };
}
...
export default function SessionDetailPage({
  ...
  const isHeadlessMode = appSettings?.disableHeaderView === true;
  ...
  const cacheWriteUnavailable =
    ...
    effectiveCacheWrite === 0 &&
    !loadedCacheTotals.writeFieldSeen;
  const activeProvider = normalizeProvider(session.provider);
  const providerLines = getProviderLines(activeProvider, cacheWriteUnavailable);
  const cacheHint = cacheWriteUnavailable
    ? `${formatTokens(effectiveCacheRead)} read · write N/A (not reported by Codex logs)`
            ...
            <div className="min-w-0 space-y-2">
              <div className="flex items-center gap-2 flex-wrap">
                ...
                {session.pricing_status &&
                  formatPricingStatus(session.pricing_status) !== "Priced" && (
                    <Badge variant="outline" className="text-[10px]">
                      {formatPricingStatus(session.pricing_status)}
                    </Badge>
                  )}
                {session.effort_mode ? (
                  <Badge variant="outline" className="text-[10px]">
          ...
          <MetricTile
            ...
            hint={
              session.p95_latency_ms > 0
                ? `user→assistant turns (${session.latency_sample_count || 0}) · p95 ${Math.round(session.p95_latency_ms)}ms`
                : `user→assistant turns (${session.latency_sample_count || 0})`
            }
          />
        </div>
      </div>

      <div className="rounded-2xl border border-border/50 bg-card/50 shadow-sm px-4 py-3">
        <div className="flex items-center gap-2">
          <div className="flex h-6 w-6 items-center justify-center rounded-lg border border-border/40 bg-background/60">
            <Info size={13} className="text-muted-foreground/80" />
          </div>
          <div className="text-xs font-medium">Provider Metrics</div>
          <Badge variant="secondary" className="text-[10px] uppercase">
            {activeProvider}
          </Badge>
        </div>
        <ul className="mt-2 space-y-1 text-xs text-muted-foreground">
          {providerLines.map((line) => (
            <li key={line} className="flex items-start gap-2">
              <span className="mt-1 h-1 w-1 rounded-full bg-muted-foreground/70 shrink-0" />
              <span>{line}</span>
            </li>
          ))}
        </ul>
        ...
        <div className="flex flex-wrap items-center gap-3">
          <Tooltip>
            ...
            <TooltipContent side="top" className="max-w-xs text-xs">
              Loads all older transcript pages automatically for this session.
            </TooltipContent>
          </Tooltip>
        </div>
      </div>
