...
import { listCodexInstructions } from "@/lib/codex/skills";
import {
  convertAgentTargets,
  convertHookTargets,
    ...
    if (artifactType === "skill") {
      ...
      if (body.source.kind === "skill") {
        ...
        if (srcProvider === "codex") {
          const inst = listCodexInstructions().find(
            (s) =>
              s.name === sourceRef.name &&
              ((sourceRef.projectPath && s.projectPath === sourceRef.projectPath) ||
                (!sourceRef.projectPath && s.visibility === "global")),
          );
          if (!inst) {
