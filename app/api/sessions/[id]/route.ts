...
export async function DELETE(
  ...
  const cleanup = deleteSessionsWithCleanup([id]);
...
export async function PATCH(
  ...
  if (action === "compress") {
    db.prepare(
      "UPDATE sessions SET compressed_at = ? WHERE id = ? AND compressed_at IS NULL",
    ).run(new Date().toISOString(), id);
  } else {
    db.prepare(
      "UPDATE sessions SET compressed_at = NULL WHERE id = ? AND compressed_at IS NOT NULL",
    ).run(id);
  }
