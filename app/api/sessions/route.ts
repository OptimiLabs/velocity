  ...
  if (groupByTask) {
    ...
    if (parentIds.length > 0) {
      ...
      const children = normalizeSessionRows(
        db
          .prepare(
            `SELECT * FROM sessions WHERE parent_session_id IN (${placeholders}) ORDER BY created_at ASC`,
          )
...
export async function DELETE(request: Request) {
  ...
  const deleteRowsTx = db.transaction((sessionIds: string[]) => {
    let deleted = 0;
  ...
  const deleted = deleteRowsTx(rows.map((row) => row.id));
