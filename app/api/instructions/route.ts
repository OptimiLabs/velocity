...
function resolveSkillGuideProvider(
  provider: ProviderTargetMode,
): SkillGuideProvider {
...
function skillProviderLabel(provider: ProviderTargetMode): string {
  if (provider === "codex") return "Codex CLI";
  if (provider === "gemini") return "Gemini CLI";
  if (provider === "all") return "cross-provider";
  return "Claude Code";
}
...
- Add safety checks — which actions need user confirmation before execution`
      ...
      const guideProvider = resolveSkillGuideProvider(targetProvider);
      const guideLabel = skillProviderLabel(targetProvider);
      const skillGuide = getSkillCreatorGuide(guideProvider);

      let fullPrompt = `<skill_creation_guide>
${skillGuide}
</skill_creation_guide>

Generate a ${guideLabel} skill called "${name}".
...
- Include a clear "When not to use" section
- Include concrete, step-by-step workflow instructions
- Include validation/verification checks for successful execution
- Include failure handling / fallback guidance
- Include an explicit output contract (what artifacts or decisions the skill must produce)
- Keep it practical and useful for real engineering workflows in this ecosystem
- Use these section headings exactly once: "When to use", "When not to use", "Workflow", "Validation", "Failure handling", "Output contract"

The body after frontmatter should be a complete, well-structured skill definition that ${guideLabel} can follow as instructions.`;

      const result = provider
        ? provider === "codex-cli"
          ? {
              content: await aiGenerate(fullPrompt, { timeoutMs: 180_000 }),
              tokensUsed: 0,
              cost: 0,
            }
          : await editWithAI({
