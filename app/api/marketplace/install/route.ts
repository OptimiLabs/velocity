...
function upsertProviderMcpServer(
  ...
  if (provider === "claude" && state.disabled[key]) {
...
function writeSkillForProvider(
  ...
  if (provider === "codex") {
    const filePath = saveCodexInstruction(safeName, toPortableSkillContent(content));
    return basename(filePath, ".md");
  }
  if (provider === "gemini") {
    const filePath = saveGeminiSkill(safeName, toPortableSkillContent(content));
    return basename(filePath, ".md");
  }
...
function trackPluginInstall(
  ...
  const key = `${targetProvider}:${name}@${marketplaceRepo.split("/").pop() || ""}`;
  (data.plugins as Record<string, unknown>)[key] = {
    name,
    targetProvider,
    marketplaceRepo,
...
export function trackPluginUninstall(
  ...
  const filePath = join(CLAUDE_DIR, "plugins", "installed_plugins.json");
  try {
    const data = JSON.parse(readFileSync(filePath, "utf-8"));
    const plugins = (data.plugins || {}) as Record<string, unknown>;
    const nameCandidates = [
      name,
      normalizeMarketplaceInstallName(name),
    ];
    const key = Object.keys(plugins).find(
      (k) =>
        nameCandidates.some(
          (candidate) =>
            k.startsWith(`${targetProvider}:${candidate}@`) ||
