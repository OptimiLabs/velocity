...
function getModelRule(provider: BuilderProvider): string {
  ...
  if (provider === "openai") {
    const models = OPENAI_API_MODEL_OPTIONS.map((model) => `"${model.id}"`).join(", ");
    return `The "model" should be one of: ${models}`;
  }
  if (provider === "google") {
    const models = GEMINI_MODEL_OPTIONS.map((model) => `"${model.id}"`).join(", ");
    return `The "model" should be one of: ${models}`;
  }
  return `The "model" should be a valid runtime model identifier for the selected provider`;
}

function getExampleModel(provider: BuilderProvider): string {
  if (provider === "claude-cli" || provider === "anthropic") return "sonnet";
  if (provider === "openai") return "gpt-5";
  if (provider === "google") return "gemini-2.5-pro";
  return "model-id";
}

function buildSystemPrompt(provider: BuilderProvider): string {
  const modelRule = getModelRule(provider);
  const exampleModel = getExampleModel(provider);
  return `You are an agent builder assistant. Help the user create a coding agent by understanding their requirements and generating a configuration.
...
- Build practical agents for real engineering work (implementation, review, testing, debugging, release safety)
- The prompt should include: mission, boundaries, numbered workflow, output format, and failure/escalation behavior
- The prompt should be structured with explicit sections (Mission, Inputs, Workflow, Validation, Output Contract, Escalation)
- Prefer workflows that can compose with MCP/tools/hooks when appropriate
