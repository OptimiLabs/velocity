...
function getAgentProviderProfile(targetProvider: ProviderTargetMode): {
  label: string;
  modelRule: string;
  exampleModel: string;
  formatHint: string;
  ...
  switch (targetProvider) {
    case "codex":
      return {
        label: "Codex CLI",
        modelRule:
          'The "model" should be a valid Codex/OpenAI model identifier (for example: "o3", "o4-mini", "gpt-5", "codex-mini-latest").',
        exampleModel: "o3",
        formatHint:
          "This configuration should read naturally for Codex CLI usage and Codex agent files.",
      };
    case "gemini":
      return {
        label: "Gemini CLI",
        modelRule:
          'The "model" should be a valid Gemini model identifier (for example: "gemini-2.5-pro", "gemini-2.5-flash").',
        exampleModel: "gemini-2.5-pro",
        formatHint:
          "This configuration should read naturally for Gemini CLI usage and Gemini agent files.",
      };
    case "all":
      return {
        label: "cross-provider (Claude + Codex + Gemini)",
        modelRule:
          'Use a portable model string if provided, or omit "model" when uncertain so downstream provider templates can adapt cleanly.',
        exampleModel: "o3",
        formatHint:
          "This configuration will be converted for multiple providers, so keep tool choices and workflow wording provider-neutral.",
      };
    case "claude":
    default:
      return {
        label: "Claude Code",
        modelRule: 'The "model" should be one of: "opus", "sonnet", "haiku".',
        exampleModel: "sonnet",
        formatHint:
          "This configuration should read naturally for Claude Code agent files.",
      };
  }
}

function buildSystemPrompt(targetProvider: ProviderTargetMode): string {
  const profile = getAgentProviderProfile(targetProvider);
  return `You generate ${profile.label} agent configurations as JSON. Output ONLY valid JSON, no markdown fences, no commentary.
...
Quality requirements for the generated agent:
- The prompt must include a concrete mission and operating boundaries.
- Include an explicit, numbered workflow with decision points.
- Include expected inputs/context and expected outputs/reporting format.
- Include an explicit output contract (how findings, diffs, risks, and next steps are reported).
- Include validation gates/checklists before completion.
- Include guardrails (what NOT to do) and escalation/failure behavior.
- Keep instructions practical for real engineering work: code review, implementation, testing, and incident/debug loops.
- Prefer ecosystem-aware workflows that can integrate with tools, MCP servers, hooks, and workflows when relevant.

Example output:
{
  "name": "security-reviewer",
  "description": "Reviews code changes for security vulnerabilities and unsafe patterns",
  "model": "${profile.exampleModel}",
...
The "prompt" field is the most important — write specific domain procedures, not generic advice. Do NOT write prompts that say "be thorough", "ask for help", or "ensure quality". Instead, write step-by-step workflows with concrete actions and completion criteria the agent should take.`;
}
...
export async function POST(request: Request) {
  try {
    ...
    const raw = await aiGenerate(userPrompt, {
      system: buildSystemPrompt(targetProvider),
