...
function CommandsPageContent() {
  ...
  const queryCommandParam = searchParams.get("command");
  const queryTypeParam = searchParams.get("type");
  const initialType = queryTypeParam;
  ...
  const [selectedType, setSelectedType] = useState<"command" | "skill">(
    initialType === "skill" || initialType === "command"
      ? initialType
      : "command",
  );
  ...
  useEffect(() => {
    if (querySearchParam !== null) {
      setSearch((prev) => (prev === querySearchParam ? prev : querySearchParam));
    }
    if (queryCommandParam) {
      setSelectedCommand((prev) =>
        prev === queryCommandParam ? prev : queryCommandParam,
      );
      if (queryTypeParam === "skill" || queryTypeParam === "command") {
        setSelectedType((prev) => (prev === queryTypeParam ? prev : queryTypeParam));
      }
    }
  }, [querySearchParam, queryCommandParam, queryTypeParam]);
  ...
  return (
    <PageContainer fullHeight>
      <PageScaffold
        title="Commands"
        subtitle="Browse built-in CLI commands and provider-scoped custom skills, then inspect handlers and metadata."
...
function CommandsPageFallback() {
  return (
    <PageContainer fullHeight>
      <PageScaffold
        title="Commands"
        subtitle="Loading commands..."
        className="flex min-h-0 flex-1 flex-col"
        bodyClassName="flex-1 min-h-0"
      >
        <div className="h-full min-h-0 rounded-2xl border border-border/70 bg-card/95 shadow-sm" />
      </PageScaffold>
    </PageContainer>
  );
}
...
export default function CommandsPage() {
  return (
    <Suspense fallback={<CommandsPageFallback />}>
      <CommandsPageContent />
    </Suspense>
  );
}
