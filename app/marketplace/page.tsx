...
function MarketplacePageContent() {
  ...
  const installPackageAllProviders = useCallback(
    ...
    [getItemKey, installPackage, installPackageForProvider],
  );
  ...
  const detailSupportedProviders = detailItem
    ...
    : [];
  const canInstallAllProviders =
    !!detailItem &&
    !detailItem.installed &&
    detailSupportedProviders.length > 1;
  const installAllProvidersLabel = canInstallAllProviders
    ? `Install to ${detailSupportedProviders
        .map(getMarketplaceProviderLabel)
        .join(" + ")}`
    : undefined;
  ...
  return (
    <PageContainer>
      ...
      <PluginDetailDialog
        ...
        onInstallAll={
          canInstallAllProviders && detailItem
            ? (components) => handleInstallAllProviders(detailItem, components)
            : undefined
        }
        installAllLabel={installAllProvidersLabel}
