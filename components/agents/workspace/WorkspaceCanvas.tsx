...
function clampMenuPosition(
  ...
  const maxLeft = Math.max(
    CONTEXT_MENU_MARGIN,
    window.innerWidth - menuWidth - CONTEXT_MENU_MARGIN,
  );
  ...
  return {
    left: Math.min(Math.max(CONTEXT_MENU_MARGIN, x), maxLeft),
    top: Math.min(Math.max(CONTEXT_MENU_MARGIN, y), maxTop),
  };
}
...
export function WorkspaceCanvas({
  ...
  return (
    <div style={{ width: "100%", height: "100%" }} className="relative">
      ...
      {/* Edge right-click context menu */}
      {edgeMenu && mounted && (() => {
        const position = clampMenuPosition(edgeMenu.x, edgeMenu.y, 160, 40);
        return createPortal(
          <div
            className="fixed z-[1000] bg-popover border border-border rounded-md shadow-lg py-1 min-w-[140px]"
      ...
      {nodeMenu && (() => {
        ...
        const actionCount =
          (onEditAgent ? 1 : 0) +
          (onDuplicateAgent ? 1 : 0) +
          (onRemoveFromWorkspace ? 1 : 0) +
          (onDeleteAgent && canDelete ? 1 : 0);
        const hasDeleteDivider = !!(onDeleteAgent && canDelete && actionCount > 1);
        const estimatedHeight = actionCount * 32 + (hasDeleteDivider ? 9 : 0) + 8;
        const position = clampMenuPosition(nodeMenu.x, nodeMenu.y, 188, estimatedHeight);
