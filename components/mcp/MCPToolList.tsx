            ...
            <div className="flex items-center gap-1.5 shrink-0">
              ...
              {usage && usage.totalTokens > 0 && (
                <Badge
                  ...
                  title="Total tokens"
                >
                  {formatTokens(Math.round(usage.totalTokens))} tok
                </Badge>
              )}
              {usage && usage.totalCalls > 0 && usage.totalTokens > 0 && (
                <Badge
                  variant="outline"
                  className="text-micro shrink-0 tabular-nums"
                  title="Average tokens per call"
                >
                  {formatTokens(Math.round(usage.avgTokensPerCall))} tok/call
                </Badge>
              )}
              {usage && usage.estimatedCost > 0 && (
                <Badge
                  variant="outline"
                  className="text-micro shrink-0 tabular-nums"
              ...
              {usage && usage.totalCalls > 0 && usage.estimatedCost > 0 && (
                <Badge
                  variant="outline"
                  className="text-micro shrink-0 tabular-nums"
                  title="Average cost per call"
                >
                  {formatCost(usage.avgCostPerCall)}/call
                </Badge>
              )}
              {usage?.lastUsed && (
