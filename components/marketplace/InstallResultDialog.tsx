...
export function InstallResultDialog({ result, open, onClose }: Props) {
  ...
  const agentsPath =
    provider === "codex"
      ...
      : provider === "gemini"
        ? "~/.gemini/agents/ (legacy: ~/.gemini/velocity/agents/)"
        : "~/.claude/agents/";
  const skillsPath =
    provider === "codex"
      ? "~/.codex/skills/"
      : provider === "gemini"
        ? "~/.gemini/skills/ (legacy: ~/.gemini/velocity/skills/)"
        : "~/.claude/skills/";
            ...
            <span>
              ...
              {hasSkillsOrCommands &&
                (provider === "claude"
                  ...
                  : provider === "codex"
                    ? " Use /skills to launch installed skills, or mention $skill-name."
                    : " Use Gemini CLI skill tooling to run installed skills.")}
            </span>
          </div>
        </div>
