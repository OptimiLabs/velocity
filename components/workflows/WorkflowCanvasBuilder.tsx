...
export function WorkflowCanvasBuilder({
  ...
  const [aiPromptExpanded, setAiPromptExpanded] = useState(true);
  ...
  // Auto-show AI prompt preview when opening a workflow detail page
  // (and when a generated plan first becomes available).
  useEffect(() => {
    if (workflow?.generatedPlan) {
      setAiPromptExpanded(true);
    }
  }, [workflowId, workflow?.generatedPlan]);
  ...
  const setSelected = useCallback(
    ...
    [layoutMode, setDetailMode, setRightOpen],
  );

  const clearSelected = useCallback(() => {
    setSelectionState((prev) => (prev === null ? prev : null));
    if (rightPanelOpen) {
      setRightOpen(layoutMode, false);
    }
  }, [layoutMode, rightPanelOpen, setRightOpen]);
  ...
  const handleDuplicateWorkspaceAgent = useCallback(
    (agent: Agent, selectionId?: string) => {
      const sourceInstanceId =
        selectionId && selectionId.includes("__i")
          ? selectionId
          : workspaceAgents.find((id) => parseInstanceId(id) === agent.name);
      const duplicateInstanceId = generateInstanceId(agent.name);

      try {
        const key = "agent-canvas-positions";
        const positions = JSON.parse(localStorage.getItem(key) || "{}") as Record<
          string,
          { x: number; y: number }
        >;
        const source = sourceInstanceId ? positions[sourceInstanceId] : undefined;
        if (source && Number.isFinite(source.x) && Number.isFinite(source.y)) {
          positions[duplicateInstanceId] = {
            x: source.x + 42,
            y: source.y + 42,
          };
          localStorage.setItem(key, JSON.stringify(positions));
        ...
        /* ignore position copy errors */
      }

      addToWorkspace(duplicateInstanceId);
      setSelected("agent", duplicateInstanceId);
  ...
  const handlePromoteWorkflowAgent = useCallback(
    async (name: string) => {
      try {
        const res = await fetch(
  ...
  const detailPanelContent = (
    <DetailPanel
      ...
      onCloneAgent={() => {}}
      onDuplicateAgent={handleDuplicateWorkspaceAgent}
    ...
    >
      ...
      <CommandDeployDialog
        ...
        onDeployed={() => {
          ...
          queryClient.invalidateQueries({ queryKey: ["skills", workflowProvider] });
        }}
      />
      <ArtifactConvertDialog
        ...
        sourceProvider={workflow.provider ?? "claude"}
        title={`Convert Workflow${workflow?.name ? `: ${workflow.name}` : ""}`}
        description="Generate provider-specific workflow variants with model and effort parity remapped for each runtime."
        getSource={() => {
          if (!workflow) return null;
          return {
            kind: "inline" as const,
            data: {
              provider: workflow.provider ?? "claude",
              name: workflow.name,
        ...
        onSaved={() => {
          queryClient.invalidateQueries({ queryKey: ["workflows"] });
          toast.success("Workflow conversion save complete");
        }}
      />
    </div>
  );
