...
function workflowSkillLabel(name: string, provider?: ConfigProvider): string {
                    ...
                    >
                      ...
                      <td className="py-2 px-3" onClick={(e) => e.stopPropagation()}>
                        {(() => {
                          ...
                          if (names.length === 0) return <span className="text-[12px] text-muted-foreground/60">—</span>;
                          return (
                            <div className="flex items-center gap-1">
                              {names.slice(0, 2).map((name) => (
                                <Link key={name} href={`/agents?search=${encodeURIComponent(name)}`}>
                                  <Badge variant="outline" className="text-meta gap-1 hover:opacity-80 transition-opacity cursor-pointer">
                                    <Bot size={9} />
                                    {name}
                                  </Badge>
                                </Link>
                                ...
                                <TooltipProvider>
                                  <Tooltip>
                                    <TooltipTrigger asChild>
                                      <span className="text-micro font-medium text-muted-foreground cursor-default">+{names.length - 2}</span>
                                    </TooltipTrigger>
                                    <TooltipContent side="bottom" align="start">
                                      <div className="flex flex-col gap-1">
                                        {names.slice(2).map((n) => (
                                          <span key={n} className="flex items-center gap-1.5 text-[12px]">
                                            <Bot size={9} /> {n}
                                          </span>
                                        ))}
                                      </div>
                        ...
                        <div className="space-y-1">
                          {wf.commandName ? (
                                ...
                                <TooltipTrigger asChild>
                                  <Link href={`/skills?search=${encodeURIComponent(wf.commandName)}`}>
                                    <Badge variant="violet" className="text-meta gap-1 hover:opacity-80 transition-opacity cursor-pointer">
                                      <Sparkles size={9} />
                                      {workflowSkillLabel(wf.commandName, wf.provider)}
                                    </Badge>
                                  </Link>
                                </TooltipTrigger>
                                <TooltipContent>
                                  {wf.provider === "codex" ? (
                                    <p>
                                      This workflow is synced as the
                                      {" "}
                                      <span className="font-mono">{wf.commandName}</span>
                                      {" "}
                                      skill for Codex. Use{" "}
                                      <span className="font-mono">/skills</span> to
                                      launch it, or mention{" "}
                                      <span className="font-mono">${wf.commandName}</span>.
                                    </p>
                                  ) : (
                                    <p>
                                      This workflow is synced as
                                      {" "}
                                      {workflowSkillLabel(wf.commandName, wf.provider)}
                                      {" "}
                                      for {workflowProviderLabel(wf.provider)}.
                                      Run {workflowSkillLabel(wf.commandName, wf.provider)} in
                                      the console to start it.
                                    </p>
                                  )}
                                </TooltipContent>
                              </Tooltip>
                            ...
                            <TooltipProvider>
                              <Tooltip>
                                <TooltipTrigger asChild>
                                  <Badge variant="violet" className="text-meta gap-1 cursor-default">
                                    <Sparkles size={9} />
                                    Skill enabled
                                  </Badge>
                                </TooltipTrigger>
                                <TooltipContent>
                                  <p>
                                    This workflow is synced as a skill but has no
                                    runnable name yet. Set a name to invoke it.
                                  </p>
                                </TooltipContent>
                              </Tooltip>
                            </TooltipProvider>
