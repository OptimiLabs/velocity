...
export function SkillMergeSheet({
  ...
  const handleSave = async (archiveOriginals: boolean) => {
    ...
    try {
      ...
      toast.success(
        archiveOriginals
          ? `Created "${name}" — invoke with /${name}. Archived ${skills.length} originals`
          : `Created "${name}" — invoke with /${name}`,
      );
      onSuccess();
      handleClose();
    } catch (e) {
  ...
  return (
    <Sheet open={open} onOpenChange={(v) => !v && handleClose()}>
      ...
      >
        ...
        {/* Actions footer */}
        {hasResult && (
          <div className="shrink-0 border-t border-border/30 pt-3 flex items-center gap-2">
            <Button
              size="sm"
