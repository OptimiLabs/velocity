...
import { calculateCostDetailed, formatCost } from "@/lib/cost/calculator";
import {
...
function MessageBubble({ msg }: { msg: Message }) {
  ...
  const usageReasoning = getUsageReasoningTokens(usageRecord);
  const fallbackUsageCost = getUsageCostUsd(usageRecord);
  const fallbackEstimate = model
    ...
    : null;
  const costMeta = msg.cost;
  const hasServerCostMeta = typeof costMeta === "object" && costMeta !== null;
  const messageCost =
    hasServerCostMeta
      ? typeof costMeta?.usd === "number"
        ? costMeta.usd
        : null
      : fallbackUsageCost !== null
        ? fallbackUsageCost
        : fallbackEstimate?.status === "priced"
          ? fallbackEstimate.cost
          : null;
  const messageCostConfidence: "reported" | "estimated" | "unpriced" | "none" =
    costMeta?.confidence ??
    (fallbackUsageCost !== null
      ? "reported"
      : fallbackEstimate?.status === "unpriced"
        ? "unpriced"
        : fallbackEstimate
          ? "estimated"
          : "none");
  const messageCostIsEstimated = messageCostConfidence === "estimated";
  ...
  const perToolCost =
    toolUsageCount > 0 &&
    messageCost !== null &&
    messageCostConfidence !== "unpriced"
        ...
        >
          ...
          {usageReasoning > 0 && (
            <span
              ...
              title="Reasoning output tokens (included in output/billable output)."
            >
              {usageReasoning.toLocaleString()} reasoning
            </span>
          )}
          {typeof messageCost === "number" && (
            <span
              className="tabular-nums rounded-full border border-border/40 bg-background/60 px-1.5 py-0.5"
              title={
                messageCostConfidence === "reported"
                  ? "Reported usage cost."
                  : messageCostConfidence === "estimated"
                    ? "Estimated from model pricing and token usage."
                    : "Unpriced model. Cost is excluded until pricing is configured."
              }
            >
              {messageCostConfidence === "estimated" ? "~" : ""}
          ...
          {messageCostConfidence === "unpriced" && (
            <span
              className="rounded-full border border-amber-500/30 bg-amber-500/10 px-1.5 py-0.5 text-[10px] text-amber-700 dark:text-amber-300"
              title="This model is not in the pricing snapshot, so no estimated cost is shown."
            >
              unpriced
            </span>
          )}
