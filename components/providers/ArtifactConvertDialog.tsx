...
type ConversionSource =
  ...
  | { kind: "workflow"; id: string; provider?: "claude" | "codex" | "gemini" };
...
export function ArtifactConvertDialog({
  ...
  const inferredSourceProvider = useMemo(() => {
    ...
    if (
      currentSource.kind === "agent" ||
      currentSource.kind === "skill" ||
      currentSource.kind === "workflow"
    ) {
  ...
  useEffect(() => {
    ...
    if (!selectableProviders.includes(targetProvider as ConfigProvider)) {
      setTargetProvider(selectableProviders[0] ?? "claude");
    }
