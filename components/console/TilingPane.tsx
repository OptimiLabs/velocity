...
export const TilingPane = memo(function TilingPaneInner({
  ...
  const {
    ...
    movePaneAction,
    isDraggingPane,
    focusedPaneId: storeFocusedPaneId,
    terminals: storeTerminals,
    maximizedPaneId,
    activeSessionId: storeActiveSessionId,
    activePaneId: storeActivePaneId,
  ...
  const activePaneId = groupActivePaneId !== undefined ? groupActivePaneId : storeActivePaneId;
  const focusedPaneId = groupFocusedPaneId !== undefined ? groupFocusedPaneId : storeFocusedPaneId;
  const activeSessionId = groupActiveSessionId !== undefined ? groupActiveSessionId : storeActiveSessionId;
  ...
  const isLeafVisible = useCallback(
    ...
    [terminals, effectiveSessionId, session?.kind, session?.id, activePaneId],
  );
    ...
    <Group
      ...
      onLayoutChanged={(layout) => {
        ...
        if (values.length === 2 && values[0] > 0 && values[1] > 0) {
          updatePaneSizes(node.id, [values[0], values[1]]);
        }
      }}
    >
      <Panel
