...
export function TerminalPanel({
    ...
    registerTerminalHandler(terminalId, (msg) => {
      ...
      try {
        ...
        if (msg.type === "pty:output" && msg.data) {
          ...
          // Update lifecycle state to active
          try {
            useConsoleLayoutStore.getState().updateTerminalMeta(terminalId, { terminalState: "active" });
          } catch {}

          // Track OSC 133 prompt marks
          if (parseOsc133(msg.data)) {
          ...
          // Update lifecycle state to exited
          try {
            useConsoleLayoutStore.getState().updateTerminalMeta(terminalId, {
          ...
          // Update lifecycle state to dead
          try {
            useConsoleLayoutStore.getState().updateTerminalMeta(terminalId, { terminalState: "dead" });
          } catch {}
        }
      } catch {
  ...
  useEffect(() => {
    ...
    const boot = (retries = 0) => {
      ...
      const savedBuffer = serializedBuffers.get(terminalId);
      if (savedBuffer) {
        t.write(savedBuffer);
    ...
    return () => {
      ...
      if (term) {
        ...
        if (stillExists && wrapper) {
          ...
          terminalDomCache.set(terminalId, {
            wrapper,
            term,
            fitAddon: fitAddon!,
          ...
          try {
            ...
            if (serialized && serialized.length <= MAX_SERIALIZE_BYTES) {
              serializedBuffers.set(terminalId, serialized);
              if (serializedBuffers.size > MAX_SERIALIZED_TERMINALS) {
                const oldest = serializedBuffers.keys().next().value;
                if (oldest) serializedBuffers.delete(oldest);
              }
