...
export function ConsoleLayout({
  ...
  const hydrated = useConsoleLayoutStore((s) => s._hydrated);

  // Use group-specific props if provided, otherwise fall back to store's derived fields
  const storePaneTree = useConsoleLayoutStore((s) => s.paneTree);
  const storeActivePaneId = useConsoleLayoutStore((s) => s.activePaneId);
  const storeTerminals = useConsoleLayoutStore((s) => s.terminals);
  const layoutMode = useConsoleLayoutStore((s) => s.layoutMode);
  const isDraggingPane = useConsoleLayoutStore((s) => s.isDraggingPane);
  const addTerminal = useConsoleLayoutStore((s) => s.addTerminal);
  const removeTerminal = useConsoleLayoutStore((s) => s.removeTerminal);
  const updateTerminalMeta = useConsoleLayoutStore((s) => s.updateTerminalMeta);

  const paneTree = groupPaneTree ?? storePaneTree;
  const activePaneId =
  ...
  const groupTabOrder = useConsoleLayoutStore(
    useShallow((s) => {
      const gid = groupId ?? s.activeGroupId ?? Object.keys(s.groups)[0];
      return gid ? s.groups[gid]?.tabOrder : undefined;
    }),
  );
  ...
  const activeSessionId = useConsoleLayoutStore((s) => s.activeSessionId);
  ...
  const setActivePaneIdForSync = useConsoleLayoutStore((s) => s.setActivePaneId);
  const storeActiveGroupId = useConsoleLayoutStore((s) => s.activeGroupId);
  useEffect(() => {
    if (
      layoutMode !== "tiling" &&
