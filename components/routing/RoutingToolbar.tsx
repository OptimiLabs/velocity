...
function entrypointLabel(ep: RoutingEntrypoint): string {
  if (!ep.projectRoot) return `${ep.label} (global)`;
  const projectName = ep.projectRoot.split("/").pop() || ep.projectRoot;
  return `${projectName} · ${ep.label}`;
}
        ...
        <div className="shrink-0 flex items-center gap-1.5">
          ...
          <TooltipProvider delayDuration={200}>
            <Tooltip>
              ...
              <TooltipContent>
                Index {providerLabel} instruction files ({entrypointFileName}) and
                map their references into a graph
              </TooltipContent>
            </Tooltip>
          </TooltipProvider>
