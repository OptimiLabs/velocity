...
export function useMultiConsole() {
  ...
  const layoutGroupCount = useConsoleLayoutStore(
    (s) => Object.keys(s.groups).length,
  );
  const layoutTerminalCount = useConsoleLayoutStore((s) => {
    let count = 0;
  ...
  useEffect(() => {
    if (!hydrated || !layoutHydrated) return;
  ...
  }, [hydrated, layoutHydrated, layoutGroupCount, layoutTerminalCount]);
