  ...
  persist(
    (set, get) => ({
      ...
      updateTerminalMeta: (terminalId, updates) =>
        set((state) => {
          ...
          for (const [gid, g] of Object.entries(state.groups)) {
            if (g.terminals[terminalId]) {
              return updateGroup(state, gid, (group) => ({
                terminals: {
                  ...group.terminals,
                  [terminalId]: { ...group.terminals[terminalId], ...updates },
                },
              }));
            }
          }
          ...
          return updateActiveGroup(state, (group) => ({
            terminals: {
              ...group.terminals,
              [terminalId]: group.terminals[terminalId]
                ? { ...group.terminals[terminalId], ...updates }
      ...
      setFocusedPane: (paneId) =>
        set((state) =>
          updateActiveGroup(state, () => ({ focusedPaneId: paneId })),
        ),
            ...
            const updateSizes = (n: PaneNode): PaneNode => {
              ...
              if (n.kind === "split") {
                return {
                  ...n,
                  children: [
                    updateSizes(n.children[0]),
                    updateSizes(n.children[1]),
                  ],
                };
              }
              return n;
            };
            return { paneTree: updateSizes(group.paneTree) };
          }),
        ),
