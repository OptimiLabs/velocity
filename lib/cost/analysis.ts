...
export function computeCostBreakdown(session: Session): CostBreakdown {
  ...
  if (models.length === 0) {
    ...
    const cacheReadCost =
      (session.cache_read_tokens / 1_000_000) * pricing.cacheRead;
    const cacheWriteCost =
      (session.cache_write_tokens / 1_000_000) * pricing.cacheWrite;
    return {
      inputCost,
      outputCost,
      cacheReadCost,
...
export function computeCacheEfficiency(session: Session): CacheEfficiency {
  ...
  // Compute savings per model from model_usage, or fall back to default pricing
  const parsed = parseJson<Record<string, ParsedModelUsage>>(
    session.model_usage,
    {},
  );
