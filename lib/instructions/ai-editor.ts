...
export async function editWithAI(request: EditRequest): Promise<EditResult> {
  ...
  if (provider === "codex-cli") {
    const fullPrompt = buildPrompt(prompt, originalContent);
    const content = await aiGenerate(fullPrompt, {
      provider: "codex-cli",
      timeoutMs: 120_000,
    });
    return {
...
${sourceBlocks}
  ...
  return `<instructions>
You are creating a new instruction entrypoint file (CLAUDE.md, AGENTS.md, or GEMINI.md) by combining the best elements from multiple source files.
...
export async function composeWithAI(
  ...
  if (!provider) {
    const content = await aiGenerate(composedPrompt, { timeoutMs: 120_000 });
    return {
      content,
      tokensUsed: 0,
      cost: 0,
  ...
  if (provider === "codex-cli") {
    const content = await aiGenerate(composedPrompt, {
      provider: "codex-cli",
      timeoutMs: 120_000,
    });
    return {
...
export function buildSkillMergePrompt(
  ...
  return `<instructions>
You are merging multiple CLI skills into a single, unified skill.
Analyze all the provided skills and create one cohesive skill that combines their functionality.

The merged skill should be provider-neutral by default so it can work for Claude Code, Codex CLI, and Gemini CLI.
Only introduce provider-specific behavior when it is required by the user request.
...
> {
  ...
  const result: EditResult =
    provider === "claude-cli"
      ...
      : provider === "codex-cli"
        ? {
            content: await aiGenerate(fullPrompt, {
              provider: "codex-cli",
              timeoutMs: 120_000,
            }),
            tokensUsed: 0,
