...
export function deleteWorkflow(id: string): boolean {
  const db = getDb();
  const tx = db.transaction((workflowId: string) => {
    // Keep cleanup explicit so older DBs (or setups without FK cascades)
    // still remove scoped agents when workflow rows are deleted.
    try {
      db.prepare("DELETE FROM workflow_agents WHERE workflow_id = ?").run(
        workflowId,
      );
    } catch {
      // Table may not exist on partially-migrated local DBs.
    }
    const result = db
