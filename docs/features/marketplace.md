...
4. **Installation** -- Packages can be installed directly from the marketplace. The install flow writes to the selected provider's config/artifact locations. `InstallResultDialog.tsx` shows provider-specific next steps.
...
3. Click "Install" to add it to your selected provider configuration
4. Review the installation result dialog for any issues

### Provider-specific run semantics

- **Claude**: Many installed skills are runnable as `/<name>`.
- **Codex**: Installed skills are available from `/skills` and via `$<name>` references.
- **Gemini**: Installed skills are provider-scoped artifacts; invocation follows Gemini CLI skill/extension flows.
