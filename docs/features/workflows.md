...
Visual canvas builder for designing multi-step agent orchestrations. Built on @xyflow/react, workflows let you define task graphs and save them as provider-scoped reusable skills (Claude, Codex, Gemini).
...
4. **Execution** -- Workflows can be deployed and executed, running each step in sequence or parallel according to the graph edges. The deploy flow writes provider-specific skill artifacts and keeps the workflow command name in sync.
...
- **Claude**: Saved workflow skill runs as `/<name>`.
- **Codex**: Saved workflow skill is launched via `/skills` or by mentioning `$<name>`.
- **Gemini**: Saved workflow skill is stored provider-side; invocation depends on Gemini CLI skill tooling (not generic `/<name>`).
...
- **Save Skill**: Generate/update provider-native workflow skill artifacts
