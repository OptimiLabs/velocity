...
describe("PUT /api/settings — provider dispatch", () => {
  ...
  it("returns 400 for invalid Gemini hook event", async () => {
    const req = makeRequest("http://localhost/api/settings?provider=gemini", {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
    ...
    expect(res.status).toBe(400);
    const data = await res.json();
    expect(String(data.error)).toContain("Invalid hook event");
  });

  it("returns 400 for unsupported Gemini hook type", async () => {
    const req = makeRequest("http://localhost/api/settings?provider=gemini", {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        hooks: {
          BeforeTool: [{ hooks: [{ type: "prompt", prompt: "review" }] }],
        },
      }),
    });
    const res = await PUT(req);
    expect(res.status).toBe(400);
    const data = await res.json();
    expect(String(data.error)).toContain("not supported");
  });
});
