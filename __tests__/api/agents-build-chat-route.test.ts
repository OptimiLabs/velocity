...
describe("POST /api/agents/build-chat (route)", () => {
  ...
  it("supports non-CLI providers over the same SSE contract", async () => {
    ...
    const generationOptions = aiGenerateMock.mock.calls[0]?.[1] as
      | { system?: string }
      | undefined;
    expect(generationOptions?.system).toContain("gpt-5.3-codex");
    expect(generationOptions?.system).toContain(
      "Mission, Inputs, Workflow, Validation, Output Contract, Escalation",
    );
    expect(generationOptions?.system).not.toContain(
      'The "model" should be one of: "opus", "sonnet", "haiku"',
    );
    expect(events).toEqual([
      {
        type: "text",
