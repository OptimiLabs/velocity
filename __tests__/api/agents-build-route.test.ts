...
describe("POST /api/agents/build (route)", () => {
  ...
  it("allows non-Claude agent models and preserves them in generated configs", async () => {
    ...
    const options = aiGenerateMock.mock.calls[0]?.[1] as
      | { model?: string; system?: string }
      | undefined;
    expect(options).toMatchObject({
      model: undefined,
    });
    expect(options?.system).toContain("Codex CLI");
    expect(options?.system).toContain("validation gates/checklists");
    expect(options?.system).not.toContain("You generate Claude Code");
  });

  it("uses Gemini-native generation prompt when targetProvider=gemini", async () => {
    aiGenerateMock.mockResolvedValueOnce(
      '{"name":"gemini-helper","description":"Gemini helper","prompt":"Do gemini work","tools":["Read"]}',
    );
    ...
    const req = new Request("http://localhost/api/agents/build", {
      ...
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        description: "Build a Gemini helper",
    ...
    const options = aiGenerateMock.mock.calls[0]?.[1] as
      | { system?: string }
      | undefined;
    expect(options?.system).toContain("Gemini CLI");
    expect(options?.system).toContain("gemini-2.5-pro");
  });
