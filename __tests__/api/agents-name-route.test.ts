...
const detachAttachmentsForTargetMock = vi.fn();
...
vi.mock("@/lib/db/instruction-files", () => ({
  detachAttachmentsForTarget: detachAttachmentsForTargetMock,
}));
...
describe("api/agents/[name] provider parity", () => {
  beforeEach(() => {
    ...
    detachAttachmentsForTargetMock.mockReset();
    dbRunMock.mockReset();
  });
  ...
  it("DELETE removes only provider-targeted agent artifacts", async () => {
    ...
    expect(detachAttachmentsForTargetMock).toHaveBeenCalledWith(
      "agent",
      "codex-agent",
    );
