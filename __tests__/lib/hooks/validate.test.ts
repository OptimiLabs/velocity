...
describe("validateHookConfig", () => {
  ...
  test("warns that PermissionRequest is conditional", () => {
    const result = validateHookConfig("PermissionRequest", {
      type: "command",
      command: "echo permission",
    });
    expect(result.warnings.some((w) => /conditional/i.test(w))).toBe(true);
  });

  test("accepts valid Gemini command hook", () => {
  ...
  test("rejects unsupported Gemini hook type", () => {
    const result = validateHookConfig(
      "BeforeTool",
      {
        type: "prompt",
    ...
    expect(result.errors.some((err) => /not supported/i.test(err))).toBe(true);
  });
});
