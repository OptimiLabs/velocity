...
describe("gemini/paths", () => {
  ...
  it("GEMINI_SKILLS_DIR points to ~/.gemini/skills", async () => {
    const { GEMINI_SKILLS_DIR } = await import("@/lib/gemini/paths");
    expect(GEMINI_SKILLS_DIR).toBe(
  ...
  it("projectGeminiSkillsDir returns <project>/.gemini/skills", async () => {
    const { projectGeminiSkillsDir } = await import("@/lib/gemini/paths");
    expect(projectGeminiSkillsDir("/my/project")).toBe(
      "/my/project/.gemini/skills",
    );
  });

  it("getGeminiSkillDirs prefers native path and keeps legacy fallback", async () => {
    const { getGeminiSkillDirs } = await import("@/lib/gemini/paths");
    expect(getGeminiSkillDirs()).toEqual([
      path.join(os.homedir(), ".gemini", "skills"),
      path.join(os.homedir(), ".gemini", "velocity", "skills"),
    ]);
  });
});
