...
describe("calculateCost", () => {
  ...
  it("returns 0 for unknown model (unpriced)", () => {
...
describe("calculateCostFromStats", () => {
  ...
  it("skips unknown models in fallback estimate", () => {
...
describe("OpenAI model pricing", () => {
  ...
  it("should have pricing for gpt-5 codex models", () => {
    ...
    const latest = MODEL_PRICING["gpt-5.3-codex"];
    ...
    expect(latest.input).toBe(1.75);
    expect(latest.output).toBe(14.0);
    expect(latest.cacheRead).toBe(0.175);
  ...
  it("returns 0 for unknown codex variants", () => {
    const cost = calculateCost("gpt-5.9-codex", 1_000_000, 1_000_000, 0, 0);
    expect(cost).toBe(0);
  });
